...
// Ambil data dari database (contoh: semua data dari tabel 'perangkat')
$query = $db->prepare("SELECT * FROM perangkat");
$query->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['submit'])) {
        ...
        if (!empty($nama) && !empty($tanggal_lahir) && !empty($no_kontak) && !empty($alamat) && isset($gender)) {
            ...
            $getLastIdQuery = $db->query("SELECT MAX(id_perangkat) AS last_id FROM perangkat");
            ...
            $insertQuery = $db->prepare("INSERT INTO perangkat (id_perangkat, nama, tanggal_lahir, no_kontak, alamat, gender_id_gender) VALUES (?, ?, ?, ?, ?, ?)");
    ...
    if (isset($_POST['edit'])) {
        ...
        $updateQuery = $db->prepare("UPDATE perangkat SET nama=?, tanggal_lahir=?, no_kontak=?, alamat=?, gender_id_gender=? WHERE id_perangkat=?");
    ...
    if (isset($_POST['hapus'])) {
        ...
        $deleteQuery = $db->prepare("DELETE FROM perangkat WHERE id_perangkat=?");
    ...
    <main id="content">
        <section id="section" class="section">
            <div class="container" id="container">
                ...
                <h2 class="heading1 text-center">DATA PERANGKAT KANTOR KELURAHAN</h2>
                                ...
                                <?php foreach ($warga as $data) : ?>
                                    <tr>
                                        <td class="checkbox-column">
                                            <input type="checkbox" name="selected_ids[]" value="<?= $data['id_perangkat'] ?>" data-id="<?= $data['id_perangkat'] ?>">
                                        </td>
                                        <td class="id"><?= $data['id_perangkat'] ?></td>
    ...
    <script>
        $(document).ready(function() {
            ...
            editButton.on('click', function() {
                ...
                editForm.find('#edit_gender').val(gender);
